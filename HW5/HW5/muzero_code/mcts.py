...
def select_child(config, node, min_max_stats):
    ...
    for a, c in node.children.items():
        score = ucb_score(config, node, c, min_max_stats)
        if score > best_score:
            best_score = score
            action = a
            child = c
...
def expand_root(node, actions, network, current_state):
    ...
    state_array = current_state[0] if isinstance(current_state, tuple) else current_state
    input_array = np.expand_dims(state_array, axis=0)
...
def expand_node(node, actions, network, parent_state, parent_action):
    ...
    Return: value
    """
    value, reward, policy_logits, hidden_state = network.recurrent_inference(parent_state, parent_action)
    #print(hidden_state)
    node.hidden_representation = hidden_state
    node.reward = reward
    softmax_policy = network._softmax(policy_logits)[0]
...
def softmax_sample(visit_counts, temperature):
    ...
    counts = visit_counts[:, 0]
    actions = visit_counts[:, 1]
    if temperature == 0:
        return actions[np.argmax(counts)]
    else:
        num_probs = [count**(1/temperature) for count in counts]
        denom = sum(num_probs)
        probs = [num_prob/denom for num_prob in num_probs]
